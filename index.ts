...
server.tool("index_documents", "Add documents from specified path for RAG Indexing", {
  path: z.string().describe("Path containing .txt and .md files to index")
}, async ({ path }) => {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Error indexing documents: ${error instanceof Error ? error.message : String(error)}`,
        },
      ],
    };
  }
...
server.tool("query_documents", "Query indexed documents using RAG", {
  query: z.string().describe("The question to search documents for"),
  k: z.number().optional().describe("Number of chunks to return (default: 15)")
}, async ({ query, k = 15 }) => {
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Error querying documents: ${error instanceof Error ? error.message : String(error)}`,
        },
      ],
    };
  }
...
server.tool("remove_document", "Remove a specific document from the index by file path", {
...
server.tool("remove_all_documents", "Remove all documents from the index", {
  confirm: z.boolean().describe("Confirmation to remove all documents (must be true)")
}, async ({ confirm }) => {
  if (!confirm) {
    return {
      content: [
  ...
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Error removing all documents: ${error instanceof Error ? error.message : String(error)}`,
        },
      ],
    };
  }
...
server.tool("list_documents", "List all document paths in the index", {}, async () => {
  try {
